...
class ThoughtData(BaseModel):
    ...
    evaluationScore: float | None = Field(
        None, ge=0, le=10, description="Adversarial score 0-10."
    )
    status: str = Field("active", description="Node lifecycle status.")
    depth: int = Field(..., ge=0, description="Depth in the tree (root=0).")
    adversarialRounds: int = Field(
        0, ge=0, description="Number of GAN refinement rounds completed."
    )
    refinementHistory: list[dict[str, Any]] | None = Field(
        None, description="Discriminator feedback per round."
    )
...
class EvaluationResult(BaseModel):
    ...
    adversarial_rounds: int = Field(
        0, ge=0, description="Total adversarial rounds completed."
    )
    history: list[dict[str, Any]] = Field(
        default_factory=list, description="Evaluation history per round."
    )
